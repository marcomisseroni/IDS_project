...
class EKF:
    ...
    def update_step(
            ...
            id_a: int,
            id_b: int,
            own_phi_override: np.ndarray = None):
        N = type(self).agent_id
        gamma = [None] * N
        ...
        # gamma_a/gamma_b embed inv(phi_a)/inv(phi_b) as of when the info reply
        # was computed remotely, so applying them here requires that SAME phi to
        # cancel correctly - not whatever phi has accumulated locally since then
        # if this agent's own predict_check_callback kept ticking in the meantime.
        # own_phi_override lets the caller pass the frozen snapshot for that case;
        # third-party agents (self.agent_id not in {id_a, id_b}) have no such
        # cancellation requirement and keep using their own live self.phi.
        own_phi = own_phi_override if own_phi_override is not None else self.phi
        self.state = self.state + own_phi @ self.gamma @ r_a
        self.P = self.P - own_phi @ self.gamma @ self.gamma.T @ own_phi.T
        for i in range(N):
            for j in range(i + 1, N):

...
class EKF(Node):
    ...
    def __init__(self, enc_weight, imu_weight, initial_state, R, Q, dt = 0.3, dx = 0, dy = 0, dtheta = 0):
        ...
        self.subscription = self.create_subscription(String, 'enc', self.enc_listener_callback, 10)
        self.subscription = self.create_subscription(String, 'imu', self.imu_listener_callback, 10)
        self.subscription = self.create_subscription(String, 'lidar', self.lidar_listener_callback, 10)
        self.subscription = self.create_subscription(String, 'admin', self.admin_listener_callback, 10)
    ...
    def timer_callback(self):
        ...
        if self.new_enc_data and self.new_imu_data and self.new_lidar_data:
            ...
            self.new_imu_data = False
            self.new_lidar_data = False
        msg.data = "[%f, %f, %f]" % (self.state[0], self.state[1], self.state[2])
        self.publisher.publish(msg)
        self.get_logger().info('Publishing: "%s"' % msg.data)

    def enc_listener_callback(self, msg):
        self.enc_v = float(msg.data.split(",")[0])
        self.enc_w = float(msg.data.split(",")[1])
        self.new_enc_data = True

    def imu_listener_callback(self, msg):
        self.imu_w = float(msg.data)
        self.new_imu_data = True

    def lidar_listener_callback(self, msg):
        self.lidar_meas[0] = float(msg.data.split(",")[0])
        self.lidar_meas[1] = float(msg.data.split(",")[1])
        self.lidar_meas[2] = float(msg.data.split(",")[2])
